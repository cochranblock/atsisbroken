...
- [x] **D**   `cdp::Session::fill` + `verify` — write value, dispatch
      input/change events, re-snapshot 250ms later for Workday
      re-render defense. Implemented as
      `run_loop::verify_and_retry`. After all fills land, waits
      250ms (matches Workday's destroy-and-recreate cycle ~150-
      200ms), reads back every filled field, retries ONE per
      blanked field. Two failures → surface to user, never silent.
      Live smoke verified against a Workday-pattern HTML fixture
      (destroy-and-recreate parent on first input event):
      "re-render verifier: 1 field(s) blanked after fill — 1
      re-filled successfully, 0 still empty".
